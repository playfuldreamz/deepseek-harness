...
* `Could not read Username for 'https://github.com'` on `git push` in the sandbox — WSL has no `gh` token, `~/.git-credentials`, or Windows Credential Manager in that environment; push from a normal Windows terminal where `gh auth` / Credential Manager has the `playfuldreamz` token, or set `https://TOKEN@github.com/...` for that push and restore the URL after.
